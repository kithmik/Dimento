    ...
    <div class="card m-2">
        <div class="card-block p-0">
            <div class="list-group">
                @if(auth()->check())
                    <form action="/comment/{{ $object->id }}" method="POST" data-id="{{ $object->id }}"  class="p-3 commenting-form">
                        {{ csrf_field() }}
                        <div class="md-form">
                                    ...
                                    <div class="row rendered-text">
                                        <div class="col-md-2">
                                            <a href="/user/{{ $comment->user->profile_pic }}" target="_blank">
                                                <img style="max-height: 165px; min-width: 50px;" class=" avatar {{--user_image--}}" data-id="{{ $comment->user->id }}" src="{{ starts_with($comment->user->profile_pic, 'http')?$comment->user->profile_pic:(!empty($comment->user->profile_pic)?'/storage/'.$comment->user->profile_pic:($comment->user->gender == 1?'/imgs/default_img/default_profile_pic_male.jpg':'/imgs/default_img/default_profile_pic_female.jpg')) }}" alt="{{ $comment->user->first_name." ".$comment->user->middle_name." ".$comment->user->last_name }}">
                                            </a>
                                        </div>
                                        <div class="col-md-8">
                                            <div class="row">
                                        ...
                                        <div class="col-md-2">
                                            <small class="text-muted"  data-toggle="tooltip" data-placement="top" title="{{ $comment->getTimeAttr() }}">{{ $comment->getAgeAttr() }}</small>
                                        </div>
                                        <div class="col-md-1">
                                            @if(auth()->check() && (auth()->user()->id == $comment->user_id || auth()->user()->id == $comment->object->user_id ))
                                                <small class="text-muted">
                                                    <button type="button" class="btn-floating red  delete-comment" onclick="event.preventDefault();
                                                            document.getElementById('delete-comment-form-of-{{ $comment->id }}').submit();">
                                                        <i class="fa fa-trash" aria-hidden="true"></i>
                                                    </button>

                                                    <form id="delete-comment-form-of-{{ $comment->id }}" action="/comment/{{ $comment->id }}/delete" method="POST" style="display: none;">
                                                        {{ csrf_field() }}
                                                    </form>
